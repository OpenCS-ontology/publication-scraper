...
from scpe_scraper.rdf_namespaces import PRISM_2_1
...
def doi_api_decoder(data: bytes) -> PaperDoiResponse:
    parsed = lightrdf.RDFDocument(
        io.BytesIO(data), parser=lightrdf.turtle.PatternParser
    )
    ...
    def find_authors(turtle: lightrdf.RDFDocument) -> List[AuthorDoiResponse]:
        ...
        author_uris = map(
            lambda t: t[0], list(turtle.search_triples(None, RDF.type, FOAF.Person))
        )
        ...
        for author_uri in author_uris:
            ...
            if len(other_author_uri) != 0:
                ...
                orcid = other_uri[
                    where_orcid + len(ORCID_STRING) + 1 : len(other_uri) - 1
                ]
            ...
            author = AuthorDoiResponse(
                orcid=orcid, given_name=given_name, family_name=family_name, name=name
            )
